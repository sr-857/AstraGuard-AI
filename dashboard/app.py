...
from classifier.fault_classifier import classify, get_fault_severity, get_fault_description
from logs.timeline import read_recent, log_event
from state_machine.state_engine import StateMachine, decide_action
...
def update_telemetry_history(telemetry_data: Dict, anomaly_score: float, 
                            fault_type: str, system_state: str) -> None:
    """Update telemetry history with new data point."""
    max_history = 100  # Keep last 100 samples
    
    # Add new data
    st.session_state.history["timestamp"].append(telemetry_data.get("timestamp", time.time()))
...
def create_telemetry_charts() -> alt.Chart:
    ...
    voltage_chart = alt.Chart(df).mark_line(
    ...
    temperature_chart = alt.Chart(df).mark_line(
    ...
    gyro_chart = alt.Chart(df).mark_line(
        color='green', strokeWidth=2
    ).encode(
        x=alt.X('index:Q', title='Time Steps'),
        y=alt.Y('gyro:Q', title='Gyro (rad/s)', scale=alt.Scale(domain=[-0.1, 0.1]))
    ).properties(height=120)
    
    anomaly_chart = alt.Chart(df).mark_line(
...
def create_system_status_panel() -> None:
    ...
    st.markdown(f"<div style='background-color: {state_color}; color: white; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold;'>{current_state}</div>", unsafe_allow_html=True)
    ...
    with col2:
        anomaly_rate = (st.session_state.anomaly_count / max(st.session_state.total_samples, 1)) * 100
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.info(f"Telemetry: {'🟢 Running' if st.session_state.telemetry_process else '🔴 Stopped'}")
    ...
    if st.session_state.telemetry_process:
        ...
        try:
            ...
            if line:
                ...
                if is_anomalous or injected_fault:
                    ...
                    if st.session_state.state_machine.check_recovery_complete():
                        response = st.session_state.state_machine.resume_normal_operation()
                        system_state = response["new_state"]
                        log_event("RECOVERY_COMPLETE", f"Resumed {system_state}")
                    else:
                        system_state = st.session_state.state_machine.current_state.value
                ...
                # Update history
                update_telemetry_history(telemetry_state, anomaly_score, fault_type, system_state)
            ...
            if st.session_state.telemetry_process and st.session_state.telemetry_process.poll() is not None:
                st.session_state.telemetry_process = None
