...
def plot_cube_solid(ax: Axes3D, R: np.ndarray, alpha: float = 0.3) -> None:
    ...
    for i, face in enumerate(faces):
        ...
        ax.add_collection3d(
            Poly3DCollection(poly, alpha=alpha, facecolor=face_colors[i], edgecolor='black')
        )
...
def plot_coordinate_axes(ax: Axes3D, R: np.ndarray) -> None:
    ...
    for i, (axis, color, label) in enumerate(zip(rotated_axes, colors, labels)):
        ax.plot3D([0, axis[0]], [0, axis[1]], [0, axis[2]], 
                 color=color, linewidth=3, label=f'{label}-axis')
        ax.text(axis[0], axis[1], axis[2], label, color=color, fontsize=12, fontweight='bold')
...
def main() -> None:
    ...
    try:
        duration = int(input("Enter animation duration in seconds (default 30): ") or "30")
