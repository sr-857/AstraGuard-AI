...
def classify(telemetry_state: Dict[str, Optional[float]]) -> str:
    ...
    try:
        ...
        current = telemetry_state.get("current")
        ...
        # Validate data types
        if not all(isinstance(x, (int, float)) for x in [voltage, temperature, gyro, wheel_speed]):
            return "sensor_fault"
...
def validate_telemetry_for_classification(telemetry_state: Dict[str, Optional[float]]) -> Dict[str, any]:
    """
    ...
    validation_result["can_classify"] = (
        ...
        len(validation_result["invalid_values"]) == 0
    )
...
def main() -> None:
    ...
    test_cases = [
        {
            ...
            "data": {"voltage": 7.9, "current": 0.55, "temperature": 24, "gyro": 0.01, "wheel_speed": 480}
        },
        {
            "name": "Power fault",
            "data": {"voltage": 6.5, "current": 0.55, "temperature": 24, "gyro": 0.01, "wheel_speed": 480}
        },
        {
            "name": "Thermal fault",
            "data": {"voltage": 7.9, "current": 0.55, "temperature": 35, "gyro": 0.01, "wheel_speed": 480}
        },
        {
            "name": "Attitude fault",
            "data": {"voltage": 7.9, "current": 0.55, "temperature": 24, "gyro": 0.08, "wheel_speed": 480}
        },
        {
            "name": "Sensor fault",
            "data": {"voltage": 7.9, "current": 0.55, "temperature": 24, "gyro": 0.01, "wheel_speed": None}
        }
    ]
    ...
    for test_case in test_cases:
        fault_type = classify(test_case["data"])
